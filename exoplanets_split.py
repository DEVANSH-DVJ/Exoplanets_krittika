...
class system(object):
    def __init__(self, file_name, time_split=100, img_split=100, n=1.0):
        try:
            ...
            self.u = data['u']
            period_constant = (2 * np.pi) / ((scipy.constants.G * self.star_mass * 1.988e30)**0.5)
            for planet in self.planets:
                planet['period'] = period_constant * \
                    ((planet['semi-major'] * self.star_radius * 6.96e8)**1.5) / (36*24)  # days
...
if __name__ == '__main__':
    ...
    exoplanets.timespan = exoplanets.timespan/100
    max_time = max(exoplanets.timespan)
    fig = plt.figure(figsize=(20, 12))
    ax = fig.add_subplot(111)
    ax.scatter(exoplanets.timespan, lum, s=1, c='b')
    ...
    print('Time : {} seconds; {}'.format(round(time.time() - start, 2), 'Made the main plot'))

    for i in range(10):
        ax.set_xlim(max_time*i*0.1, max_time*(i+1)*0.1)
        ax.set_ylim(ylim)
        plt.savefig(fname='plots/exoplanets/{} {} {}.jpeg'.format(params_file[:-5], n, i), dpi=500, pad_inches=0.0, quality=100)
        print('Time : {} seconds; {}'.format(round(time.time() - start, 2), 'Made the {} plot'.format(i)))
