...
def update_graph(iteration, total, scat, ax, time_coord, n_planets):
    locs = []
    x, y, z = [], [], []
...
axes.append(fig.add_axes([0.1, 0.6, 0.3, 0.3], projection='3d'))  # ax[1]
...
anim = animation.FuncAnimation(fig, update_graph, frames=len(timespan), interval=2,
                               repeat=True, fargs=(len(timespan), scatters, axes, time_coord, n_planets), blit=False)
